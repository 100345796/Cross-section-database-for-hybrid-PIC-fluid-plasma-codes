...
def _select_fileroot_filename_unknown(root,Type, Specie1, Specie2, reaction): 
    ...
    if Type == 'BINARY':
        if len(Database[root][Type][Specie1][Specie2][reaction]) == 1:
            for file_name in Database[root][Type][Specie1][Specie2][reaction].keys():#database.txt,...
                file_root = root+'/'+Type +'/'+Specie1+'/'+Specie2+'/'+reaction+'/'+file_name
        else:
            options = []
            for file_name in Database[root][Type][Specie1][Specie2][reaction].keys():#database.txt,...
                if len(Database[root][Type][Specie1][Specie2][reaction]) != 1 and file_name[0:8] == 'database':
                    options = options + [file_name]
            file_used = options[0]
            file_root = root+'/'+Type +'/'+Specie1+'/'+Specie2+'/'+reaction+'/'+file_used

    elif Type == 'UNARY':
        for specie1 in Database[root][Type].keys():# e-, Ar, Kr,...
            if specie1==Specie1:
                for reaction in Database[root][Type][specie1].keys():#de_ex
                    if len(Database[root][Type][specie1][reaction]) == 1:
                        for file_name in Database[root][Type][specie1][reaction].keys():#database.txt,...
                            file_root = root+'/'+Type +'/'+specie1+'/'+reaction+'/'+file_name
                    else:
                        options = []
                        for file_name in Database[root][Type][specie1][reaction].keys():#database.txt,...
                            if len(Database[root][Type][specie1][reaction]) != 1 and file_name[0:8] == 'database':
                                options = options + [file_name]
                        file_used = options[0]
